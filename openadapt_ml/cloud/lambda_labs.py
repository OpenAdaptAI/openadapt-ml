...
def main():
    ...
    if args.command == "list":
        ...
        # Refresh function for background polling
        def refresh_loop():
            from openadapt_ml.training.trainer import TrainingState, TrainingConfig, generate_training_dashboard
                ...
                try:
                    ...
                    if instances:
                        ...
                        if status and status.get("step", 0) > 0:
                            ...
                            config = TrainingConfig(
                                num_train_epochs=state.total_epochs,
        ...
        # Start refresh thread if requested
        if args.refresh:
            try:
                # Verify we can connect to Lambda
                instances = client.list_instances()
                if instances:
                    print(f"Found Lambda instance: {instances[0].ip}")
                    refresh_thread = threading.Thread(target=refresh_loop, daemon=True)
                    refresh_thread.start()
                    print(f"Started background refresh (every {args.interval}s)")
                else:
                    print("No Lambda instances found - refresh disabled")
            except Exception as e:
                print(f"Could not connect to Lambda: {e}")
